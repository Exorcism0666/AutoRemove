...
(async () => {
    ...
    data.forEach(async (obj) => {
        console.log(`[${data.indexOf(obj) + 1}/${data.length}] ${colors.blue("issue")} ${colors.yellow(`#${obj.number}`)}`)
        ...
        const ready = commets.data.filter((obj) => {
            if (obj.user?.login == ("coolplaylinbot") && obj.body?.includes("## For moderators")) {
                return obj
            }
        });
        if (ready.length === 0) {
            ...
            console.log(`${colors.green(`Comment in issue`)} ${colors.yellow(
                `#${obj.number}`,
