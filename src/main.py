...
# Add manosim.gitify to Update List
   ...
   # Add gopass.gopass-jsonapi to Update List
    ...
    JSON = requests.get("https://api.github.com/repos/adventuregamestudio/ags/latest", verify=False, headers=Headers[1]).json()["assets"]
   ...
   # Add Rinrab.AOVpnManager to Update List
    id = "Rinrab.AOVpnManager"
    JSON = requests.get("https://api.github.com/repos/rinrab/aovpn-manager/releases/latest", verify=False, headers=Headers[1]).json()["assets"]
    Version = requests.get("https://api.github.com/repos/rinrab/aovpn-manager/releases/latest", verify=False, headers=Headers[1]).json()["tag_name"]
    Urls = [each["browser_download_url"] for each in JSON if each["browser_download_url"].endswith(".msi")]
    if not version_verify(Version, id):
        report_existed(id, Version)
    elif do_list(id, Version, "verify"):
