...
def command(komac: pathlib.Path, id: str, urls: str, version: str, token: str) -> str:
    Commands = "{} update --identifier {} --urls {} --version {} --submit --token {} --submit".format(komac.__str__(), id, urls, version, token)
    return Commands
...
# Add apankrat.byenow to Update List
    id = "apankrat.byenow"
    JSON = requests.get("https://api.github.com/repos/apankrat/byenow/releases/latest", verify=False, headers=Headers[1]).json()["assets"]
    Version = re.search(r'\d+(\.\d+)+', requests.get("https://api.github.com/repos/apankrat/byenow/releases/latest", verify=False, headers=Headers[1]).json()["tag_name"]).group()
    Urls = [each["browser_download_url"] for each in JSON if each["browser_download_url"].endswith(".zip")]
    if not version_verify(Version, id):
        report_existed(id, Version)
    elif do_list(id, Version, "verify"):
