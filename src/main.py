...
# Add manosim.gitify to Update List
   ...
   # Add DeterminedAI.CLI to Update List
    id = "DeterminedAI.CLI"
    JSON = requests.get("https://api.github.com/repos/istio/istio/releases/latest", verify=False, headers=Headers[1]).json()["assets"]
    Version = requests.get("https://api.github.com/repos/istio/istio/releases/latest", verify=False, headers=Headers[1]).json()["tag_name"]
    Urls = [each["browser_download_url"] for each in JSON if each["browser_download_url"].endswith(".zip")]
    if not version_verify(Version, id):
        report_existed(id, Version)
    elif do_list(id, Version, "verify"):
