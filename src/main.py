...
# Add manosim.gitify to Update List
   ...
   # Add Databricks.DatabricksCLI to Update List
    id = "Databricks.DatabricksCLI"
    JSON = requests.get("https://api.github.com/repos/databricks/cli/releases/latest", verify=False, headers=Headers[1]).json()["assets"]
    Version = requests.get("https://api.github.com/repos/databricks/cli/releases/latest", verify=False, headers=Headers[1]).json()["tag_name"]
    Urls = [each["browser_download_url"] for each in JSON if ("zip" in each["browser_download_url"]) and ("windows" in each["browser_download_url"])]
    if not version_verify(str_pop(Version, 0), id):
         report_existed(id, Version)
    elif do_list(id, Version, "verify"):
        report_existed(id, Version)
   ...
   # Add Bitwarden.Bitwarden to Update List
    id = "Bitwarden.Bitwarden"
    JSON = requests.get("https://api.github.com/repos/bitwarden/clients/releases/latest", verify=False, headers=Headers[1]).json()["assets"]
    Version = re.sub(r'[^\d.]', '', requests.get("https://api.github.com/repos/bitwarden/clients/releases/latest", verify=False, headers=Headers[1]).json()["tag_name"]) if tag_name.startswith("desktop-") else None
    Urls = [each["browser_download_url"] for each in JSON if each["browser_download_url"].endswith(".exe")]
    if not version_verify(str_pop(Version, 0), id):
         report_existed(id, Version)
    elif do_list(id, Version, "verify"):
